...
public class BoardPersistRepository {
    ...
    public void update(Long boardId, BoardRequest.UpdateDTO updateDTO) {
        ...
        // board -> 영속성 컨텍스트 1차 캐쉬에 key=vale 값이 저장 되어 있다.
    ...
    public Board findById(Long id) {
        // 1차 캐시 활용
//        Board board = em.find(Board.class, id); // 영속성 상태로 조회
//        return board;
...
//    }
    ...
    public Board save(Board board) {
        ...
        // 3. 트랜잭션 커밋 시점에 내부적으로 Insert 쿼리 실행
        //      - 이때 영속성 컴텍스트의 변경 사항이 자동으로 DB에 반영됨
        //      - board 객체의 id 필드에 자동으로 생성된 값이 설정됨
        // insert --> DB --> (pk 값을 알 수 있다)
        // select --> DB --> (할당된 pk 값 조회)

        // 4. 영속 상태로 된 board 객체를 반환
        //      - 이 시점에는 자동으로 board id 멤버변수에 db pk 값이 할당된 상태이다
        return board;
    }
